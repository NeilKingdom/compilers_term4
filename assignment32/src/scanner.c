...
Token tokenizer(void) {
	...
	pheonix_int copyState = 0;     /* copy of state for temp usage */
	pheonix_int prevState = 0;     /* previous state of the FSM */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		case '>':
			...
			if(newc == '=') {
				...
				currentToken.code = BIT_OP_T;
				currentToken.attribute.codeType = OP_BSR;
				return currentToken;
			}
			retract(sourceBuffer);
			currentToken.code = REL_OP_T;
		...
		case '<':
			...
			if (newc == '=') {
				...
				currentToken.code = BIT_OP_T;
				currentToken.attribute.codeType = OP_BSL;
				return currentToken;
			}
			retract(sourceBuffer);
			currentToken.code = REL_OP_T;
		...
		case '&':
			...
			currentToken.code = BIT_OP_T;
			currentToken.attribute.codeType = OP_BWA;
			return currentToken;
		case '|':
			newc = getChar(sourceBuffer);
			if (newc == '|') {
			...
			currentToken.code = BIT_OP_T;
			currentToken.attribute.codeType = OP_BWO;
			return currentToken;
		case '!':
				newc = getChar(sourceBuffer);
				if(newc == '!') {
		...
		default: 
			...
			/* Need to preserve previous state because in Pheonix, certain final states can have recursive loops. */
			while(1) {
				if(copyState != state)
					prevState = copyState;
...
Token funcBL(pheonix_char lexeme[]) {
	...
	newlexeme = malloc((lexLength-1) * sizeof(char));

	strcpy(newlexeme, lexeme+2);
...
Token funcFL(pheonix_char lexeme[]) {
	Token currentToken = { 0 };
	pheonix_float tfloat;

	/* Remove optional (and unnecesarry) 'f' from the end of lexeme */
